...
var number2words = require( '@stdlib/number-uint64-base-number2words' ).assign;
...
function Uint64( value ) {
	...
	if ( isBigInt( value ) && BigInt.asUintN( 64, value ) === value ) {
		bigint2words( value, buffer, STRIDE, OFFSET );
	} else if ( isInteger( value ) && isBetween( value, 0, TWO_64, 'closed', 'open' ) ) {
...
setReadOnly( Uint64.prototype, 'valueOf', function valueOf() {
	if ( HAS_BIGINT ) {
		return ( BigInt( this.hi ) << BigInt( 32 ) ) | BigInt( this.lo );
	}
	return ( this.hi * TWO_32 ) + this.lo;
});
